...
export default function PokerControl({
  ...
  return (
    <div className="w-full bg-gradient-to-r from-[#031f2f] to-[#0c3342] p-4 rounded-lg shadow-lg">
      <div className="flex justify-between items-center mb-4">
        <h3 className="text-white text-xl font-bold">Your Chips: ${playerChips}</h3>
        {isLoading && <div className="text-yellow-400">Processing...</div>}
      </div>
      
      {!isPlayerTurn ? (
        <div className="text-center py-4 text-gray-300">Waiting for other players...</div>
      ) : (
        <div className="flex flex-col gap-4">
          ...
          {showBetSlider && (
            <div className="mt-4 p-4 bg-[#0a2333] rounded-lg">
              <div className="flex justify-between mb-2">
                <span className="text-white">Bet Amount: ${betAmount}</span>
                <span className="text-gray-400 text-sm">Min: ${minBetAllowed} | Max: ${maxBetAllowed}</span>
              </div>
              <input
                type="range"
                min={minBetAllowed}
